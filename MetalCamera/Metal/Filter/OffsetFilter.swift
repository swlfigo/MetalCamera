...
class OffsetFilter : BaseFilter {
    ...
    //originX 为Cropfilter的输入纹理在输出纹理上下偏移的百分比，百分比基于输出纹理的高度计算
    //如
    ...
    private var _outputFrame : CGRect = .zero
    ...
    override func newTextureAvailable(_ texture: MetalTexture) {
        ...
        if !outputFrame.equalTo(.zero) {
            ...
            let right = Float (outputFrame.origin.x) + contentSizeWidth  / Float(outputFrame.width)
